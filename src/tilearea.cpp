...
{
	assert(start < tile_map.size);
	assert(end < tile_map.size);
...
{
	assert(this->tile < tile_map.size);
	this->w = std::min<int>(this->w, tile_map.size_x - TileX(this->tile));
	this->h = std::min<int>(this->h, tile_map.size_y - TileY(this->tile));
}
