...
struct MAPSChunkHandler : ChunkHandler {
	...
	void Save() const override
	{
		...
		_map_dim_x = tile_map.size_x;
		_map_dim_y = tile_map.size_y;
...
struct MAPHChunkHandler : ChunkHandler {
	...
	void Load() const override
	{
		...
		for (TileIndex i = 0; i != size;) {
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) tile_map.raw(i++).height = buf[j];
		}
	}
	...
	void Save() const override
	{
		...
		SlSetLength(size);
		for (TileIndex i = 0; i != size;) {
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) buf[j] = tile_map.raw(i++).height;
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
		}
	}
};
...
struct M3LOChunkHandler : ChunkHandler {
	...
	void Load() const override
	{
		...
		for (TileIndex i = 0; i != size;) {
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) tile_map.raw(i++).m3 = buf[j];
		}
	}
	...
	void Save() const override
	{
		...
		SlSetLength(size);
		for (TileIndex i = 0; i != size;) {
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) buf[j] = tile_map.raw(i++).m3;
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
		}
	}
};
...
struct M3HIChunkHandler : ChunkHandler {
	...
	void Load() const override
	{
		...
		for (TileIndex i = 0; i != size;) {
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) tile_map.raw(i++).m4 = buf[j];
		}
	}
	...
	void Save() const override
	{
		...
		SlSetLength(size);
		for (TileIndex i = 0; i != size;) {
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) buf[j] = tile_map.raw(i++).m4;
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
		}
	}
};
...
struct MAP5ChunkHandler : ChunkHandler {
	...
	void Load() const override
	{
		...
		for (TileIndex i = 0; i != size;) {
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) tile_map.raw(i++).m5 = buf[j];
		}
	}
	...
	void Save() const override
	{
		...
		SlSetLength(size);
		for (TileIndex i = 0; i != size;) {
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) buf[j] = tile_map.raw(i++).m5;
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
		}
	}
};
...
struct MAPEChunkHandler : ChunkHandler {
		...
		if (IsSavegameVersionBefore(SLV_42)) {
			for (TileIndex i = 0; i != size;) {
				...
				for (uint j = 0; j != 1024; j++) {
					tile_map.raw_e(i++).m6 = GB(buf[j], 0, 2);
					tile_map.raw_e(i++).m6 = GB(buf[j], 2, 2);
					tile_map.raw_e(i++).m6 = GB(buf[j], 4, 2);
					tile_map.raw_e(i++).m6 = GB(buf[j], 6, 2);
				}
			}
		} else {
			for (TileIndex i = 0; i != size;) {
				SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
				for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) tile_map.raw_e(i++).m6 = buf[j];
			}
		}
	}
	...
	void Save() const override
	{
		...
		SlSetLength(size);
		for (TileIndex i = 0; i != size;) {
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) buf[j] = tile_map.raw_e(i++).m6;
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
		}
	}
};
...
struct MAP7ChunkHandler : ChunkHandler {
	...
	void Load() const override
	{
		...
		for (TileIndex i = 0; i != size;) {
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) tile_map.raw_e(i++).m7 = buf[j];
		}
	}
	...
	void Save() const override
	{
		...
		SlSetLength(size);
		for (TileIndex i = 0; i != size;) {
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) buf[j] = tile_map.raw_e(i++).m7;
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT8);
		}
	}
};
...
struct MAP8ChunkHandler : ChunkHandler {
	...
	void Load() const override
	{
		...
		for (TileIndex i = 0; i != size;) {
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT16);
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) tile_map.raw_e(i++).m8 = buf[j];
		}
	}
	...
	void Save() const override
	{
		...
		SlSetLength(size * sizeof(uint16));
		for (TileIndex i = 0; i != size;) {
			for (uint j = 0; j != MAP_SL_BUF_SIZE; j++) buf[j] = tile_map.raw_e(i++).m8;
			SlCopy(buf.data(), MAP_SL_BUF_SIZE, SLE_UINT16);
		}
	}
};
