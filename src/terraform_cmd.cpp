...
{
	/* Make sure all tiles passed to TerraformAddDirtyTile are within [0, tile_map.size] */
...
static std::tuple<CommandCost, TileIndex> TerraformTileHeight(TerraformerState *ts, TileIndex tile, int height)
{
	...
	/* Recurse to neighboured corners if height difference is larger than 1 */
	{
		...
		for (ttm = _terraform_tilepos; ttm != endof(_terraform_tilepos); ttm++) {
			...
			if (tile >= tile_map.size) continue;
			/* Make sure we don't wrap around the map */
			if (Delta(TileX(orig_tile), TileX(tile)) == tile_map.size_x - 1) continue;
			if (Delta(TileY(orig_tile), TileY(tile)) == tile_map.size_y - 1) continue;
...
std::tuple<CommandCost, Money, TileIndex> CmdTerraformLand(DoCommandFlag flags, TileIndex tile, Slope slope, bool dir_up)
{
	...
	if ((slope & SLOPE_W) != 0 && tile + TileDiffXY(1, 0) < tile_map.size) {
	...
	if ((slope & SLOPE_S) != 0 && tile + TileDiffXY(1, 1) < tile_map.size) {
	...
	if ((slope & SLOPE_E) != 0 && tile + TileDiffXY(0, 1) < tile_map.size) {
...
{
	if (start_tile >= tile_map.size) return { CMD_ERROR, 0, INVALID_TILE };
