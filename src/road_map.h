...
static inline RoadTileType GetRoadTileType(TileIndex t)
{
	...
	return (RoadTileType)tile_map.get(t).road.tile_type;
}
...
static inline RoadBits GetRoadBits(TileIndex t, RoadTramType rtt)
{
	...
	return (RoadBits)tile_map.get(t).road.road_bits;
}
...
static inline void SetRoadBits(TileIndex t, RoadBits r, RoadTramType rtt)
{
	...
	if (rtt == RTT_TRAM) {
		...
		tile_map.get(t).road.road_bits = r;
	}
}
...
{
	assert(MayHaveRoad(t));
	return (RoadType)GB(tile_map.get_e(t).road.m8, 6, 6);
}
...
{
	assert(MayHaveRoad(t));
	if (rtt == RTT_ROAD) return (Owner)GB(IsNormalRoadTile(t) ? tile_map.get(t).road.owner : tile_map.get_e(t).road.m7, 0, 5);
...
static inline void SetRoadOwner(TileIndex t, RoadTramType rtt, Owner o)
{
	if (rtt == RTT_ROAD) {
		if (IsNormalRoadTile(t))
			...
			SB(tile_map.get_e(t).road.m7, 0, 5, o);
	} else {
...
{
	assert(IsNormalRoad(t));
	return (DisallowedRoadDirections)tile_map.get(t).road.disalowed_direction;
}
...
static inline void SetDisallowedRoadDirections(TileIndex t, DisallowedRoadDirections drd)
{
	assert(IsNormalRoad(t));
	assert(drd < DRD_END);
	tile_map.get(t).road.disalowed_direction = drd;
}
...
static inline Axis GetCrossingRoadAxis(TileIndex t)
{
	...
	return (Axis) tile_map.get(t).road.crossing.axis;
}
...
static inline bool HasCrossingReservation(TileIndex t)
{
	...
	return tile_map.get(t).road.crossing.reserved;
}
...
{
	assert(IsLevelCrossingTile(t));
	tile_map.get(t).road.crossing.reserved = b;
}
...
{
	assert(IsLevelCrossing(t));
	return tile_map.get(t).road.crossing.barred;
}
...
{
	assert(IsLevelCrossing(t));
	tile_map.get(t).road.crossing.barred = barred;
}
...
{
	return HasBit(tile_map.get_e(t).road.m7, 5);
}
...
{
	ToggleBit(tile_map.get_e(t).road.m7, 5);
}
...
{
	return (Roadside)GB(tile_map.get_e(tile).road.m6, 3, 3);
}
...
{
	SB(tile_map.get_e(tile).road.m6, 3, 3, s);
}
...
{
	AB(tile_map.get_e(t).road.m7, 0, 4, 1);

	return GB(tile_map.get_e(t).road.m7, 0, 4) == 15;
}
...
static inline void TerminateRoadWorks(TileIndex t)
{
	...
	/* Stop the counter */
	SB(tile_map.get_e(t).road.m7, 0, 4, 0);
}
...
static inline DiagDirection GetRoadDepotDirection(TileIndex t)
{
	...
	return (DiagDirection)tile_map.get(t).depot.road.direction;
}
...
static inline void SetRoadTypeTram(TileIndex t, RoadType rt)
{
	...
	SB(tile_map.get_e(t).road.m8, 6, 6, rt);
}
